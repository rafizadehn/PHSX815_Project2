...
if __name__ == "__main__":
    ...
    # flag = (power>0.98)
    # index = np.where(flag==True)[0][0]

    # power analysis plot details

    plt.figure()
    plt.scatter(percentiles*N, power, c = 'crimson', alpha = 0.7, label= 'T = '+str(int(T2))+' K')
    plt.xlim([0, N])
    plt.ylim([0.05, 1])
    ...
    ## title in case you want it. remove if adding to paper.
    plt.title('Statistical Power of H1 Compared to H0 ('+str(int(T1))+' K) Per Sample Size', fontsize = 15, fontweight = "bold")
